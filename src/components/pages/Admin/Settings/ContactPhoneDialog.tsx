...
function ContactPhoneDialog({ open, data, handleClose }: Props) {
  ...
  let contactPhoneList: string[] = useMemo(() => {
    let items: string[] = [];
    if (data && data?.contact_phone_numbers) {
      items = safeJSONParse(data?.contact_phone_numbers?.value) || [];
    }
    return items;
  }, [data, open]);
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers) => {
      setDefaultHeaders();
      let payload = {
        key: data?.contact_phone_numbers?.key,
        value: [...contactPhoneList, values.contact_phone_numbers],
      };
      try {
    ...
    validationSchema: yup.object().shape({
      contact_phone_numbers: yup.string().required().label("Phone"),
    }),
  });
  ...
  const handleRemoveEmail = async (val: string) => {
    setDefaultHeaders();
    const emailValues = contactPhoneList?.filter((item) => item !== val) || [];
    let payload = {
      key: data?.contact_phone_numbers?.key,
      value: emailValues,
    };
        ...
        <Box component={"form"} onSubmit={handleSubmit}>
          <Box>
            <FormControl size="small" fullWidth sx={{ my: 1 }}>
              <FormLabel>Add phone number</FormLabel>
              <OutlinedInput
                name="contact_phone_numbers"
                type="tel"
                value={values.contact_phone_numbers}
                onChange={handleChange}
                onBlur={handleBlur}
              />
              {touched.contact_phone_numbers &&
                errors.contact_phone_numbers && (
          ...
          {contactPhoneList && contactPhoneList?.length > 0 ? (
            <Box>
