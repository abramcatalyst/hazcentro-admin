...
type Props = {
  selectedDiscount: SettingsDiscountType;
  handleClose: () => void;
};
...
function EditDiscountSetting({ selectedDiscount, handleClose }: Props) {
  ...
  let initialValues = {
    ...
    rate: selectedDiscount?.rate || 1,
    start_date: "",
    end_date: "",
    applicable_categories: [],
    is_active: convertBooleanToString(selectedDiscount?.is_active),
    override_vendor_discount: convertBooleanToString(
      selectedDiscount?.override_vendor_discount
    ),
  ...
  useEffect(() => {
    setStartDate(dayjs(selectedDiscount?.start_date));
    setEndDate(dayjs(selectedDiscount?.end_date));
